...
TF1 <- "WRKY29"
list <- mclapply(1:22, function(x) {
  
  TF1 <- merge_df$TF[x] %>% gsub("_s", "", .)
  
  wrky33 <- merge_df %>%
    filter(TF %in% c(paste0(TF1, "_s"), paste0(TF1, "_m"))) %>%
    arrange(desc(TF))
  

  seqs_s <- read_lines(wrky33$clean_reads[1]) %>% fjComm::length_adjust(101L)
  pfm_s  <- fjComm::pfm_from_seed(
    seqs_or_file = seqs_s,
    seed1 = "AAGT", gapLen = 0, seed2 = "CAAC",
    two_strands = TRUE, flankLen = 3,
    all_start_with_specified_gap = TRUE, rmdup_fg = TRUE
  )
  pfm_s %<>% apply(2, function(x) x / sum(x))
  

  seqs_m <- read_lines(wrky33$clean_reads[2]) %>% fjComm::length_adjust(101L)
  pfm_m  <- fjComm::pfm_from_seed(
    seqs_or_file = seqs_m,
    seed1 = "AAGT", gapLen = 0, seed2 = "CAAC",
    two_strands = TRUE, flankLen = 3,
    all_start_with_specified_gap = TRUE, rmdup_fg = TRUE
  ...
  C_effects <- c(C_strand_plus, C_strand_minus) %>%
    as.data.frame() %>% t() %>% set_rownames(TF1) %>% melt()
  
  abs_max <- c(abs(pfm_m[2, ] - pfm_s[2, ]), abs(pfm_m[3, ] - pfm_s[3, ])) %>%
    as.data.frame() %>% t() %>% set_rownames("size")
  ...
  plot1 <- plot_grid(prow1, prow2, blank_, ncol = 1,
                     rel_heights = c(0.60, 0.35, 0.05), greedy = FALSE)
  
  ggsave(
    filename = glue("~/2024_WRKY/11_diff_bind_all_WRKY_amp_dap_methy/plot/LOGO_{TF1}_new_version.pdf"),
    plot = plot1, width = 3.26, height = 1.34
  )
...
df$Var1 <- factor(df$Var1, levels = as.character(ord))
df <- df[df$Var1 %in% name, ]
...
plot <- ggplot(df, aes(x = Var2, y = Var1)) +
  ...
  theme_minimal() +
  labs(subtitle = "KL_dist_cutoff = 0.012, abs(value) < 0.3 ~ medium") +
  theme(
